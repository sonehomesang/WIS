...
{{-- WH override of livewire::tailwind — the ONLY change vs the vendor view is the
     current-page pill, which is now clearly highlighted (sky-600) so the active
     page is obvious. Keep in sync with vendor if Livewire updates the view. --}}
<div>
                                ...
                                @foreach ($element as $page => $url)
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                        @if ($page == $paginator->currentPage())
                                            {{-- ★ WH change: active page clearly highlighted --}}
                                            <span aria-current="page">
                                                <span class="relative z-10 inline-flex items-center px-4 py-2 -ml-px text-sm font-bold text-white bg-sky-600 border border-sky-600 cursor-default leading-5 shadow-sm">{{ $page }}</span>
                                            </span>
                                        @else
                                            <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-600 bg-white border border-gray-300 leading-5 hover:bg-sky-50 hover:text-sky-700 focus:z-10 focus:outline-none focus:border-sky-300 focus:ring ring-sky-300 transition ease-in-out duration-150" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                                {{ $page }}
                                            </button>
                                        @endif
                                    </span>

...
<div class="flex items-center gap-2" x-data="{ open: false }">
    ...
    <div class="relative" x-on:click.outside="open = false">
        ...
        <div x-show="open" x-cloak x-transition class="absolute right-0 z-30 mt-1 w-44 bg-white border border-gray-200 rounded-md shadow-lg py-1">
            <a href="{{ route('profile') }}" wire:navigate class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">ໂປຣໄຟລ໌</a>
            <button wire:click="logout" class="w-full text-start px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">ອອກ ຈາກ ລະບົບ</button>
        </div>
    </div>
</div>

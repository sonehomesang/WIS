            ...
            <div class="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/40 md:p-4">
                <div class="bg-white w-full md:max-w-2xl rounded-t-lg md:rounded-lg p-5 space-y-4 max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div><label class="block text-gray-600 mb-1">ເຈົ້າຂອງ / Owner <span class="text-xs text-gray-400">(ດຶງ ຈາກ ຜູ້ ໃຊ້)</span></label><select wire:model="ef.owner_user_id" class="w-full rounded-md border-gray-300 text-sm"><option value="">—</option>@foreach ($ownerUsers as $ou)<option value="{{ $ou->id }}">{{ $ou->display_name ?: $ou->email }}</option>@endforeach</select>@error('ef.owner_user_id')<p class="text-xs text-red-600">{{ $message }}</p>@enderror</div>

@php
    ...
    $nav = [
        ...
        ['menu' => 'deposit', 'label' => 'Deposit Material/Equipment', 'route' => '#',
    ...
    $linkActive = 'bg-gray-800 text-white font-medium';
@endphp
        ...
        <nav class="px-2 py-4 space-y-1">
            @foreach ($nav as $item)
                @if (($item['always'] ?? false) || $u->can($item['menu'].'.view'))
                    @php $active = isset($item['route']) && $item['route'] !== '#' && request()->url() === $item['route']; @endphp
